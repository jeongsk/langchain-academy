...
아래는 A2A 커뮤니티의 일원으로서 AI 에이전트가 서로 효과적으로 소통하고 협업하며 사용자와도 효과적으로 소통하고 협업할 수 있도록 A2A를 표준 프로토콜로 구축, 성문화 및 채택하는 데 도움을 주는 파트너 목록입니다 (가능한 경우 A2A 발표 또는 블로그 게시물 링크 포함).

!!! note "참고"

    A2A의 파트너가 되어 이 페이지에 귀사의 목록을 추가하거나 업데이트하는 데 관심이 있으시면, [이 양식을 제출](https://goo.gle/a2a-partner-form)하여 알려주시면 곧 연락드리겠습니다!
